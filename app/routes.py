...
from .voice_control import enviar_comando_openai, VoiceRecognitionSystem, PERSONALIDADES
from .VoiceControlService import VoiceControlService
from .plan_de_vuelo import crear_mision
from dronLink.Dron import Dron
from .dron_controls import ejecutar_mision, conectar_dron, despegar_dron, aterrizar_dron, mover_dron, armar_dron, obtener_coordenadas, desconectar_dron, obtener_datos_telemetria, detener_dron, armar_y_despegar, rotar_dron
from .ModoGlobal import connect_mqtt, mqtt_client_instance, publish_command
...
conectado=False
armado=False
voice_recognition = VoiceRecognitionSystem(r"C:\Users\Victor\Documents\WebAppVoice\vosk-model-small-es-0.42")
...
def conectar_broker():
    from .ModoGlobal import mqtt_client_instance, connect_mqtt
    try:
...
from .ModoGlobal import ultima_respuesta
import time 
...
def obtener_telemetria():
    from .dron_controls import obtener_datos_telemetria
...
def cambiar_estado2():
    try:
        ...
        from .dron_controls import cambiar_estado as cambiar_estado_dron
        resultado = cambiar_estado_dron(estado)
        

        if resultado is None:
            return jsonify({"estado": "error", "message": "La función no devolvió un resultado válido"}), 500
...
def ejecutar_plan():
    try:
        from .ModoGlobal import publish_command
...
def check_broker():
    from .ModoGlobal import mqtt_client_instance
    if mqtt_client_instance and mqtt_client_instance.is_connected():
