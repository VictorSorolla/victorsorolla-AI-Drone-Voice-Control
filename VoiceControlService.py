...
from .dron_controls import conectar_dron, armar_dron, despegar_dron, aterrizar_dron, mover_dron, desconectar_dron, armar_y_despegar, rotar_dron
from .voice_control import enviar_comando_openai, personalidad_actual
import json
import openai
...
class VoiceControlService:
    ...
    def procesar_comando_normal(self, comando):
        ...
        from .voice_control import personalidad_actual
    ...
    def _extraer_valor_numerico(self, texto):#Extrae el primer número que vaya seguido de 'metro(s)' o 'grado(s)'
        match = re.search(r'(\d+)\s*(metro|metros|grado|grados)', texto.lower())
    ...
    def ejecutar_accion(self, accion, metros, es_confirmacion=False):
        ...
        from .voice_control import personalidad_actual
        from .ModoGlobal import publish_command, mqtt_client_instance, connect_mqtt
        import time
        ...
        try:
            ...
            if accion == "conectar":
                ...
                resultado = mover_dron(direcciones[accion], metros)  
            ...
            if comando_mqtt:
                publish_command(comando_mqtt)
                
            print(f"DEBUG: Resultado de la acción: {resultado}")
            if resultado and resultado["estado"] == "success":
                print("DEBUG: Acción exitosa")
            if personalidad_actual == "pregunton":
                if es_confirmacion: 
                    prompt_accion = (
    ...
    def cambiar_personalidad(self, user_id, nueva_personalidad):
        ...
        from .voice_control import cambiar_personalidad as cambiar_personalidad_vc
